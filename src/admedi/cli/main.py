...
def pull(
    ...
    async def _pull_async() -> None:
        async with LevelPlayAdapter(cred) as adapter:
            ...
            # Generate per-app settings (country matching + shared files + networks)
            settings_path, networks_path_str, info_messages = (
                generate_app_settings(groups, profile)
            )
...
def sync(
    ...
    networks: Annotated[
        bool,
        typer.Option("--networks", help="Sync network waterfall configurations."),
    ] = False,
    dry_run: Annotated[
        bool,
    ...
    (dry-run), 2 = error.
    """
    # Build SyncScope from flags: no flags = full sync
    if not tiers and not networks:
        scope = SyncScope(tiers=True, networks=True)
    else:
        scope = SyncScope(tiers=tiers, networks=networks)
    ...
    # Load tiers from per-app settings (two-layer resolution)
    try:
    ...
    # Load network presets when scope includes networks
    network_presets: dict | None = None
    if scope.networks:
        try:
